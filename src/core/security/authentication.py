...
class JsonWebToken(Authorization):
	...
	def create_token(self, payload_data, *arg: typing.Any, **kwargs: typing.Any) -> typing.Dict:
		token = jwt.encode(
			payload={
	...
	async def validate(
		...
		try:
			...
			if _type.lower() != 'bearer':
				raise
			_decode = jwt.decode(_token, key=self.access_key, algorithms=[self.algorithm])
			_payload = _decode.get('payload', {})
			return _payload
		except jwt.exceptions.ExpiredSignatureError:
			raise Forbidden(msg='Token is expired', error_code=ErrorCode.TOKEN_EXPIRED.name)
