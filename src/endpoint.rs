...
impl HTTPEndpoint {
    ...
    pub fn method_not_allowed(&self, _request: PyObject) -> Py<PyAny> {
        Python::with_gil(|py| {
            let response = py
                .import_bound("robyn")
                .unwrap()
            ...
            let headers = py
                .import_bound("robyn")
                .unwrap()
                .call_method1("Headers", ("Content-Type: application/json",))
                .unwrap();
            let description = py.import_bound("orjson").unwrap().call_method1("dumps", (py.import_bound("orjson").unwrap().call_method1("loads", ("{\"data\": \"\", \"errors\": \"Method Not Allowed\", \"error_code\": 405}",)).unwrap(),)).unwrap();
            response
                .call_method1("new", (description, headers, 405))
                .unwrap()
                .unbind()
        })
    }
